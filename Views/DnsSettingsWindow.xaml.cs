...
public sealed partial class DnsSettingsWindow : Window
{
    ...
    {
        _owner = owner;
        _initialDnsJson = DnsPresetService.EnsureDnsBodyJson(dnsJson);
        InitializeComponent();
        ...
        Closed += OnClosed;
        Activated += DnsSettingsWindow_Activated;
    ...
    private async void DnsSettingsWindow_Activated(object sender, WindowActivatedEventArgs args)
    {
        if (_editorReady)
        {
    ...
    private async void DnsSettingsWindow_ThemeChanged(FrameworkElement sender, object args)
    {
        if (_editorReady)
        {
            await ApplyEditorThemeAsync();
    ...
    private async void FormatButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var formatted = DnsPresetService.FormatDnsBodyJson(current);
    ...
    private async void RestoreDefaultsButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        ClearError();
        await SetEditorContentAsync(DnsPresetService.CreateDefaultDnsBodyJson());
    }
    ...
    private async void SaveButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var formatted = DnsPresetService.FormatDnsBodyJson(current);
            await SetEditorContentAsync(formatted);
            Complete(DnsPresetService.BuildDnsJsonFromBody(formatted));
            Close();
        }
        catch (Exception ex)
        {
    ...
    private void Complete(string? dnsJson)
    {
        if (_completed)
        {
            return;
        ...
        _completed = true;
        _completion.TrySetResult(dnsJson);
    }
